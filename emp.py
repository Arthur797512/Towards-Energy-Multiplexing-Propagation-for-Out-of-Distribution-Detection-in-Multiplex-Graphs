...
class EMP(nn.Module):
    """
    EMP: Energy-based Multiplex Propagation
    """
    ...
    def propagation(self, e, adjs, prop_layers=1, alpha=0.5, alpha_inter=0.5):
        """
        核心能量传播层
        alpha: 层内残差系数
        alpha_inter: 层间残差系数
        ...
        # 1. Intra-structure Propagation (层内传播)
        ...
        E_intra = torch.stack(intra_propagated_list, dim=1)  # [N, M]

        # 2. Inter-structure Propagation (层间交互)
