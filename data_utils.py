...
def to_sparse_tensor(edge_index, edge_feat, num_nodes):
    """
    将 PyG 的 edge_index 转换为 torch_sparse 的 SparseTensor
    """
...
# -----------------------------------------------------------------------
#  Metrics & Evaluation Utils (核心 OOD 指标)
# -----------------------------------------------------------------------
...
def fpr_and_fdr_at_recall(y_true, y_score, recall_level=0.95, pos_label=None):
    """ 计算 FPR95 (Recall=0.95 时的 False Positive Rate) """
    ...
    negative_count = np.sum(np.logical_not(y_true))
    if negative_count == 0:
        return 0.0, thresholds[cutoff]

    return fps[cutoff] / negative_count, thresholds[cutoff]
...
# -----------------------------------------------------------------------
#  Main Evaluation Loops (极致纯净版：完全剥离 Baseline)
# -----------------------------------------------------------------------
...
def evaluate_classify(model, dataset, eval_func, criterion, args, device):
    ...
    if args.dataset in ('proteins', 'ppi'):
        valid_loss = criterion(out[valid_idx], y[valid_idx].to('cpu').to(torch.float))
    else:
        valid_out = F.log_softmax(out[valid_idx], dim=1)
        valid_loss = criterion(valid_out, y[valid_idx].squeeze(1).to('cpu'))
...
def evaluate_detect(model, dataset_ind, dataset_ood, criterion, eval_func, args, device, return_score=False):
    ...
    if isinstance(dataset_ood, list):
        ...
        try:
            ...
            id_filename = f'id_{dataset_name}_emp_{args.prop_scheme}.npy'
            ood_filename = f'ood_{dataset_name}_emp_{args.prop_scheme}.npy'

            np.save(id_filename, test_ind_score.cpu().numpy())
            np.save(ood_filename, test_ood_score.cpu().numpy())
        except Exception as e:
            pass # 发生错误时静默处理即可，保持日志纯净

        auroc, aupr, fpr, _ = get_measures(test_ind_score, test_ood_score)
        result = [auroc] + [aupr] + [fpr]

    # 3. 计算 IND 分类准确率和 Validation Loss
    ...
    if args.dataset in ('proteins', 'ppi'):
        valid_loss = criterion(out[valid_idx], dataset_ind.y[valid_idx].to('cpu').to(torch.float))
    else:
        valid_out = F.log_softmax(out[valid_idx], dim=1)
        valid_loss = criterion(valid_out, dataset_ind.y[valid_idx].squeeze(1).to('cpu'))
